...
│                                     WriteProcessor.tpl.php, QueryExtension.tpl.php,
│                                     ExportDataController.tpl.php, test/ServiceTest.tpl.php
├── Util/
│   └── EntityAttributeWriter.php  # AST-based #[ApiResource]/#[ApiFilter] injection into an
│                                     existing entity file (nikic/php-parser), used by ApiCrudMaker
└── CrudMakerBundle.php
tests/
...
| `make:kmj-api-crud` | ApiCrudMaker | Generate API Platform CRUD write-side (Input DTO, Service+Interface, WriteProcessor, optional owner-scoped query extension, a CSV-export-data Controller, optional unit test, `crud-specs/<Entity>.json` sidecar for `@kematjaya/crud-ui-generator`) for an *existing* entity. Args: `entity-class`, `owner-property` (`-` for none), `permission-prefix` (`-` to guess from entity name), `with-access-control` (bool — gates create/edit/delete/export_all via `kematjaya/access-control-bundle`'s `isGranted('<prefix>.<action>')`), `with-tests` (bool), `searchable-fields` (comma-separated field names, `-`/empty for none — drives both the export controller's search filter and the `searchable` flag per field in the spec JSON), `write-entity-attributes` (bool, default asks/true — see below). By default now DOES edit the entity file: `Kematjaya\CrudMakerBundle\Util\EntityAttributeWriter` uses `nikic/php-parser`'s format-preserving pretty-printer (the same technique `symfony/maker-bundle`'s internal `ClassSourceManipulator` uses for `make:entity`, but that class can't build attribute *arguments* that are themselves expressions like `operations: [new GetCollection(), ...]`, so this writer parses the whole target attribute block as its own snippet and transplants the resulting `AttributeGroup` nodes onto the real class instead) to add `#[ApiResource(...)]` and (if there are searchable fields) `#[ApiFilter(SearchFilter::class, ...)]` plus their `use` statements directly to the entity, leaving everything else in the file byte-for-byte untouched. Refuses (throws, caught by the maker) if the entity already carries an attribute with a colliding short name — falls back to printing the block to paste in by hand in that case (or if `write-entity-attributes` is false). The generated Service's write style is picked automatically by `ApiCrudRenderer::detectWriteMode()` (via Reflection on the entity class, no entity file changes): if the entity has a constructor accepting >= the field count AND an `update()` method, it generates `new Entity(...fields)` / `$entity->update(...fields)` calls (Note/TestArticle style, params assumed in the same order as Doctrine field mapping order — adjust manually if not); otherwise, if the entity has a zero-arg-callable constructor and a `set{Field}()` for every field (+ `set{OwnerProperty}()` if there's an owner), it generates `new Entity()` + `$entity->setField(...)` calls instead (plain `make:entity`-scaffolded style). If neither pattern is viable, generation **fails with a `\RuntimeException`** naming the missing setters — deliberately loud instead of silently generating a Service that drops data (unmatched constructor args are silently discarded by PHP, not an error) or fatals on `update()`. The export controller assumes standard `get{Field}()` getters exist for every field it exports (including the detected timestamp field) — adjust manually if getter names differ. datetime/date/time fields are skipped from the Input DTO (treated as system-managed) — see `ApiCrudRenderer::SKIPPED_TYPES`; a field named like `created*` of one of those types is still detected separately as `timestampField` (spec JSON + export "Created" column), via `ApiCrudRenderer::findTimestampField()`. The spec JSON also carries `idType` (`uuid`/`int`/`string`, from `ApiCrudRenderer::detectIdType()` reading the id column's Doctrine type) — not every entity uses UUID ids, and the frontend generator reads this to pick a matching id validator instead of hard-assuming UUID. Prints a `rate_limiter` YAML block for the export endpoint to add to `config/packages/framework.yaml` by hand — never auto-edits config files. Export endpoint is a plain Symfony controller (not an ApiPlatform operation) returning JSON `{totalItems, member}`; the frontend generator (`@kematjaya/crud-ui-generator`) is the one that turns that into an actual CSV download, mirroring `boilerplate/frontend`'s hand-written Notes feature. `bulk_delete`/`export_selected` are frontend-only permission keys (gate buttons) — no dedicated backend endpoint exists for either; bulk delete just issues one `DELETE` per selected row. |
